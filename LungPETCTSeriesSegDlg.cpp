// LungPETCTSeriesSegDlg.cpp : 实现文件
//
...
void CLungPETCTSeriesSegDlg::OnBnClickedButtonInitct()
{
	...
	//把导入的dicm数据添加选择某人待分割的CT文件夹文本编辑框到中
	m_editInitCT.SetWindowText(_T(chDir)); 
}


//点击选择该人待分割的PET文件夹按钮事件
void CLungPETCTSeriesSegDlg::OnBnClickedButtonInitpet()
{
	...
	//把导入的dicm数据添加选择该人待分割的PET文件夹文本编辑框到中
	m_editInitPET.SetWindowText(_T(chDir)); 
}


//点击存储该人序列分割后的CT文件夹按钮事件
void CLungPETCTSeriesSegDlg::OnBnClickedButtonSegct()
{
	...
	memset((char *)&bi, 0, sizeof(bi));
	bi.hwndOwner = this->m_hWnd;
	bi.lpszTitle = "选择存储该人序列分割后的CT文件夹";
	...
	//把导入的dicm数据添加存储该人序列分割后的CT文件夹文本编辑框到中
	m_editSegCT.SetWindowText(_T(chDir)); 
}


//点击存储该人序列分割后的PET文件夹按钮事件
void CLungPETCTSeriesSegDlg::OnBnClickedButtonSegpet()
{
	...
	memset((char *)&bi, 0, sizeof(bi));
	bi.hwndOwner = this->m_hWnd;
	bi.lpszTitle = "选择存储该人序列分割后的PET文件夹";
	...
	//把导入的dicm数据添加存储该人序列分割后的PET文件夹文本编辑框到中
	m_editSegPET.SetWindowText(_T(chDir)); 
}


//点击肺部PET-CT序列分割启动按钮事件
void CLungPETCTSeriesSegDlg::OnBnClickedButtonPetctseriessegStart()
{
	// TODO: 在此添加控件通知处理程序代码

	m_buttonPETCTSeriesSegStart.SetWindowText(_T("程序正在运行，请等待..."));
	m_buttonPETCTSeriesSegStart.EnableWindow(FALSE); //禁用button
	...
	if(strInitCT == "")//判断是否选择了选择某人待分割的CT文件夹
	{
		// 显示消息对话框   
		MessageBox(_T("请先选择某人待序列分割的CT文件夹！"), _T("提示"), MB_OK | MB_ICONWARNING);  
		m_buttonPETCTSeriesSegStart.EnableWindow(TRUE); //启动button
		return;
	}
	else if(strInitPET == "")//判断是否选择了选择某人待分割的PET文件夹
	{
		// 显示消息对话框   
		MessageBox(_T("请先选择该人待序列分割的PET文件夹！"), _T("提示"), MB_OK | MB_ICONWARNING); 
		m_buttonPETCTSeriesSegStart.EnableWindow(TRUE); //启动button
		return;
	}
	else if(strSegCT == "")//判断是否选择了存储该人序列分割后的CT文件夹
	{
		// 显示消息对话框   
		MessageBox(_T("请先选择存储该人序列分割后的CT文件夹！"), _T("提示"), MB_OK | MB_ICONWARNING);  
		m_buttonPETCTSeriesSegStart.EnableWindow(TRUE); //启动button
		return;
	}
	else if(strSegPET == "")//判断是否选择了存储该人序列分割后的PET文件夹
	{
		// 显示消息对话框   
		MessageBox(_T("请先选择存储该人序列分割后的PET文件夹！"), _T("提示"), MB_OK | MB_ICONWARNING);  
		m_buttonPETCTSeriesSegStart.EnableWindow(TRUE); //启动button
		return;
	}
	...
	m_buttonPETCTSeriesSegStart.EnableWindow(TRUE); //启动button
	m_buttonPETCTSeriesSegStart.SetWindowText(_T("开始PET-CT序列分割"));
...
//==============肺部PET-CT序列分割函数实现===============================
void CLungPETCTSeriesSegDlg::InitQueue(LinkQueue &Q)  //队列初始化
{
...
bool CLungPETCTSeriesSegDlg::IsEmpty(LinkQueue Q)//队列判空
{
	if(Q.front == Q.rear)
	{
		return true;
...
void CLungPETCTSeriesSegDlg::EnQueue(LinkQueue &Q,int x,int y)//入队
{
	QNode *s;
...
bool CLungPETCTSeriesSegDlg::DeQueue(LinkQueue &Q,int &x,int &y)//出队
{
...
int CLungPETCTSeriesSegDlg::LungPET_CTSeriesSeg()
{
	...
	//============读入dicm系列图片====================================================
	 ...
	 namesGenerator->SetInputDirectory( strInitCT );//输入CT_dicm图像系列所在目录

	 CString outputDirectory = strSegCT;   //输出CT_dicm图像所在的目录
	 ...
	 strPETCTSeriesSeg +=_T("待序列分割的CT和PET文件夹中有 ");
	 strPETCTSeriesSeg +=_T(strConvert);
	 strPETCTSeriesSeg +=_T(" 组CT和PET DICM图像");
	 m_editPETCTSeriesSeg_View.SetWindowText(_T(strPETCTSeriesSeg)); 
	 ...
	 PET_namesGenerator->SetInputDirectory( strInitPET );//输入PET_dicm图像系列所在目录

	 CString PET_outputDirectory = strSegPET;   //输出PET_dicm图像所在的目录
